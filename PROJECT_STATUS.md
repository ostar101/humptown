...
**Updated:** 2026-09-22
**Milestone:** M6 — Consequences — **complete in code (0.6.0)**. M5 complete (0.5.0). M5 complete in code (0.5.0). **M8 — A town worth walking — under way**, plan at `C:\Users\miika\.claude\plans\peli-tuntuu-hieman-tyls-lt-prancy-hamming.md` (twelve steps). Session A (steps 1-5) is complete. **Session B (steps 6-10, the shady spectrum) is under way: step 6 done.** Steps 7-10, then session C (steps 11-12, vouching and the new people), are next.
**Build:** green. 1064 tests, 20280 assertions with the LimeZu art installed,
~46 s. No leak warnings at exit.
